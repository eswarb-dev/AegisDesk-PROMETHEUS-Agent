...
function formatMessages(title: string, messages: BotMessageRow[]): string {
  return [title, "", ...messages.map((message) => `${formatTime(message.created_at)}  ${formatSpeaker(message)}:\n${message.text_redacted ?? message.text ?? ""}`)].join("\n\n");
}
...
function buildExport(contactId: string, messages: BotMessageRow[]): string {
  return [
    ...
    "",
    ...messages.map((message) => `[${message.created_at ?? ""}] ${formatSpeaker(message, contactId)}: ${message.text_redacted ?? message.text ?? ""}`)
  ].join("\n");
}

function formatSpeaker(message: BotMessageRow, fallbackContactId?: string): string {
  if (message.direction === "inbound") return titleCase(message.contact_id ?? fallbackContactId ?? "User");
  if (message.owner_initiated || message.sender_role === "owner" || message.message_type === "owner_relay") return "OWNER via PROMETHEUS";
  return "PROMETHEUS";
}
